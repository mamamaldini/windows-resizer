...
class WindowResizer:
    ...
    def create_widgets(self):
        ...
        for i, (text, w, h) in enumerate(presets):
            ttk.Button(
                preset_frame,
                text=text,
                command=lambda x=w, y=h: self.apply_preset(x, y)
        ...
        self.screen_label = tk.Label(
            self.root,
            text=f"🖥 Screen Resolution : {screen_w} x {screen_h}",
            fg="#6EEB83",
            bg="#1E1E2F",
            font=("Segoe UI", 10)
        )
    ...
    def resize_window(self):
        ...
        try:
            ...
            self.status.config(
                text=f"✅ Window resized to {width} x {height}",
                fg="#6EEB83"
            )
